...
pub struct ImageViewerApp {
    ...
    mouse_pos: Vec2,
    /// True while the left button is held over the image to peek at 1:1. This
    /// replaces the old click-and-drag pan: holding jumps to native size and
    /// pans while held (magnifying-glass style), snapping back to the normal
    /// fit-to-window view on release.
    peeking: bool,
        ...
        match event {
            ...
            Event::MouseButtonDown { mouse_btn: MouseButton::Left, x, y, .. } => {
                ...
                if self.show_delete_confirmation {
                    ...
                    // Hold to peek at 1:1, anchored on the click point (that image
                    // point stays under the cursor as the zoom jumps to native size).
                    self.peeking = true;
    ...
    pub fn update(&mut self, renderer: &mut Renderer) {
        ...
        if let Some(video) = &mut self.video {
            ...
            if self.is_scaled_to_fit {
                ...
                let interacting = self.peeking || self.interacted;
                if !interacting {
